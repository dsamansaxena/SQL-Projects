...
FROM ipl_match_schedule ims INNER JOIN ipl_stadium ist
ON ims.stadium_id=ist.stadium_id WHERE remarks IS NULL
GROUP BY ims.stadium_id,city
ORDER BY ims.stadium_id;
...
SELECT bid_team, team_name,COUNT(bidder_id) 'total bids'
...
SELECT  win_details,match_winner, 
CASE 
WHEN win_details LIKE '%CSK_won%' THEN '1'
WHEN win_details LIKE '%KKR_won%' THEN '4'
WHEN win_details LIKE '%RR_won%' THEN '6'
WHEN win_details LIKE '%RCB_won%' THEN '7'
WHEN win_details LIKE '%DD_won%' THEN '2'
WHEN win_details LIKE '%MI_won%' THEN '5'
WHEN win_details LIKE '%KXIP_won%' THEN '3'
WHEN win_details LIKE '%SRH_won%' THEN '8'
END winner_team_id
FROM ipl_match; 

-- Q6.	Display total matches played, total matches won and total matches lost by team along with its team name.
SELECT ist.team_id,team_name,SUM(matches_played) total_matches_played,SUM(matches_won) total_matches_won,SUM(matches_lost) total_matches_lost
FROM ipl_team_standings ist JOIN ipl_team it
ON ist.team_id=it.team_id
GROUP BY ist.team_id,team_name;

-- Q7.	Display the bowlers for Mumbai Indians team.
SELECT ips.player_id,player_name,player_role 
FROM ipl_team_players ips JOIN  ipl_player ip
ON ips.player_id=ip.player_id
...
SELECT it.team_id,team_name,COUNT(ips.player_id) 
FROM ipl_team_players ips JOIN  ipl_player ip
ON ips.player_id=ip.player_id
JOIN ipl_team it ON it.team_id=ips.team_id
WHERE player_role='all-rounder'
GROUP BY team_id
HAVING COUNT(ips.player_id)>4
ORDER BY COUNT(ips.player_id) DESC;
